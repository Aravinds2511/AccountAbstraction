...
contract SendPackedUserOp is Script {
    ...
    function generateSignedUserOperation(bytes memory callData, HelperConfig.NetworkConfig memory config, address minimalAccount) public view returns (PackedUserOperation memory) {
        ...
        //(uint8 v, bytes32 r, bytes32 s) = vm.sign(config.account, digest); //need pvt key but foundry does find it without exposing the pvt key

        userOp.signature = abi.encodePacked(r, s, v); // orders imp
        return userOp;
    }
